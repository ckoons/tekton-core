...
def update_athena(athena_path: Path, logger) -> bool:
    """
    Update Athena to use centralized services.
    
    Args:
        athena_path: Path to Athena directory
    ...
    # Find a suitable files to update
    suitable_files = find_suitable_files(athena_path, [
        ...
        if "import logging" in content:
            ...
            for pattern in logger_init_patterns:
                if pattern in content:
                    # Get module name from current content
                    module_name = f'"athena.{file_path.stem}"'
    ...
    if updated_any:
        ...
        readme_content = get_readme_content("Athena")
        
        if not write_file(readme_path, readme_content):
            logger.error(f"Failed to write README to {readme_path}")
            # Continue anyway, this is not critical
        else:
            logger.info("Created LOGGING_UPDATE.md for Athena with information about the changes")
